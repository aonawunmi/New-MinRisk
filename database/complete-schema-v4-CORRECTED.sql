-- ============================================================================
-- MinRisk v4.0 - Complete Database Schema (CORRECTED RLS POLICIES)
-- Based on SOLUTION_SPECIFICATION_DOCUMENT.md Section 4
-- ============================================================================
-- CRITICAL FIX: Added admin policies for user-scoped tables
-- This allows admins to see/manage all org data, not just their own
-- ============================================================================
...
-- ============================================================================
-- PART 10: ROW-LEVEL SECURITY (RLS) POLICIES - CORRECTED
-- ============================================================================
...
-- ============================================================================
-- Risks RLS (USER-SCOPED - Requires both user and admin policies)
-- ============================================================================

-- SELECT: Users see own risks, Admins see all org risks
...
-- INSERT: Users can insert their own risks, Admins can insert for anyone in org
...
-- UPDATE: Users can update their own risks, Admins can update all org risks
...
-- DELETE: Users can delete their own risks, Admins can delete all org risks
...
-- ============================================================================
-- Controls RLS (Cascades from Risks)
-- ============================================================================

-- SELECT: Users see controls for their risks, Admins see all org controls
...
-- INSERT: Users can insert controls for their risks, Admins can insert for any org risk
...
CREATE POLICY "Admins can insert controls for any org risk"
  ON controls FOR INSERT
  TO authenticated
  WITH CHECK (risk_id IN (SELECT id FROM risks WHERE organization_id = current_org_id()) AND is_admin());

-- UPDATE: Users can update controls for their risks, Admins can update all org controls
...
CREATE POLICY "Admins can update all org controls"
  ON controls FOR UPDATE
  TO authenticated
  USING (risk_id IN (SELECT id FROM risks WHERE organization_id = current_org_id()) AND is_admin());

-- DELETE: Users can delete controls for their risks, Admins can delete all org controls
...
CREATE POLICY "Admins can delete all org controls"
  ON controls FOR DELETE
  TO authenticated
  USING (risk_id IN (SELECT id FROM risks WHERE organization_id = current_org_id()) AND is_admin());

-- ============================================================================
-- KRI Definitions RLS (ORG-SCOPED - Single policy works for all users)
-- ============================================================================
...
-- ============================================================================
-- KRI Data Entries RLS (Inherits from kri_definitions)
-- ============================================================================
...
CREATE POLICY "Users can view their org KRI data"
  ...
  TO authenticated
  USING (kri_id IN (SELECT id FROM kri_definitions WHERE organization_id = current_org_id()));
...
CREATE POLICY "Users can manage their org KRI data"
  ...
  TO authenticated
  USING (kri_id IN (SELECT id FROM kri_definitions WHERE organization_id = current_org_id()))
  WITH CHECK (kri_id IN (SELECT id FROM kri_definitions WHERE organization_id = current_org_id()));

-- ============================================================================
-- KRI Alerts RLS (Inherits from kri_definitions)
-- ============================================================================
...
-- ============================================================================
-- KRI Risk Links RLS (Inherits from kri_definitions)
-- ============================================================================
...
-- ============================================================================
-- Control Enhancement Plans RLS (ORG-SCOPED)
-- ============================================================================
DROP POLICY IF EXISTS "Users can view their org enhancement plans" ON control_enhancement_plans;
CREATE POLICY "Users can view their org enhancement plans"
...
CREATE POLICY "Users can manage their org enhancement plans"
  ON control_enhancement_plans FOR ALL
  TO authenticated
  USING (organization_id = current_org_id())
  WITH CHECK (organization_id = current_org_id());

-- ============================================================================
-- External Events RLS (ORG-SCOPED)
-- ============================================================================
...
-- ============================================================================
-- Risk Intelligence Alerts RLS (Links to risks via risk_code)
-- ============================================================================
DROP POLICY IF EXISTS "Users can view intelligence alerts for their risks" ON risk_intelligence_alerts;
CREATE POLICY "Users can view intelligence alerts for their risks"
  ON risk_intelligence_alerts FOR SELECT
  TO authenticated
  USING (risk_code IN (SELECT risk_code FROM risks WHERE user_id = auth.uid() OR (organization_id = current_org_id() AND is_admin())));

DROP POLICY IF EXISTS "Users can manage intelligence alerts for their risks" ON risk_intelligence_alerts;
CREATE POLICY "Users can manage intelligence alerts for their risks"
  ON risk_intelligence_alerts FOR ALL
  TO authenticated
  USING (risk_code IN (SELECT risk_code FROM risks WHERE user_id = auth.uid() OR (organization_id = current_org_id() AND is_admin())))
  WITH CHECK (risk_code IN (SELECT risk_code FROM risks WHERE user_id = auth.uid() OR (organization_id = current_org_id() AND is_admin())));

-- ============================================================================
-- VaR Scale Config RLS (ORG-SCOPED)
-- ============================================================================
DROP POLICY IF EXISTS "Users can view their org VaR config" ON var_scale_config;
CREATE POLICY "Users can view their org VaR config"
  ON var_scale_config FOR SELECT
  TO authenticated
  USING (organization_id = current_org_id());

DROP POLICY IF EXISTS "Admins can manage their org VaR config" ON var_scale_config;
CREATE POLICY "Admins can manage their org VaR config"
  ON var_scale_config FOR ALL
  TO authenticated
  USING (organization_id = current_org_id() AND is_admin())
  WITH CHECK (organization_id = current_org_id() AND is_admin());

-- ============================================================================
-- Audit Trail RLS (ORG-SCOPED, Read-only)
-- ============================================================================
...
-- ============================================================================
-- Archived Risks RLS (ADMIN-ONLY)
-- ============================================================================
DROP POLICY IF EXISTS "Admins can view archived risks" ON archived_risks;
CREATE POLICY "Admins can view archived risks"
...
-- ============================================================================
-- Archived Controls RLS (ADMIN-ONLY)
-- ============================================================================
DROP POLICY IF EXISTS "Admins can view archived controls" ON archived_controls;
CREATE POLICY "Admins can view archived controls"
  ON archived_controls FOR SELECT
  TO authenticated
  USING (archived_risk_id IN (SELECT id FROM archived_risks WHERE organization_id = current_org_id()) AND is_admin());
...
CREATE POLICY "Admins can manage archived controls"
  ...
  TO authenticated
  USING (archived_risk_id IN (SELECT id FROM archived_risks WHERE organization_id = current_org_id()) AND is_admin())
  WITH CHECK (archived_risk_id IN (SELECT id FROM archived_risks WHERE organization_id = current_org_id()) AND is_admin());
...
DO $$
BEGIN
  RAISE NOTICE '✅ MinRisk v4.0 Complete Schema Migration Successful (RLS CORRECTED)!';
  RAISE NOTICE '📊 Created/Updated: 19 tables with proper RLS policies';
  RAISE NOTICE '🔒 CRITICAL FIX: Added admin policies for user-scoped tables (risks, controls)';
  RAISE NOTICE '👥 Access Control: Users see own data, Admins see all org data';
