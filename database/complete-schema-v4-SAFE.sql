-- ============================================================================
-- MinRisk v4.0 - Complete Database Schema (SAFE MIGRATION)
...
-- ============================================================================
-- PART 1: ENSURE CORE TABLES EXIST WITH ALL COLUMNS
-- ============================================================================
...
-- risks table - create if not exists
...
-- Add missing columns to risks table if they don't exist
DO $$
BEGIN
...
BEGIN
  IF (TG_OP = 'INSERT') THEN
    INSERT INTO audit_trail (
      organization_id, user_id, user_email, action, entity_type, entity_id, entity_code, new_values, timestamp
    ) VALUES (
      NEW.organization_id, auth.uid(), (SELECT email FROM auth.users WHERE id = auth.uid()),
      'create', 'risk', NEW.id, NEW.risk_code, to_jsonb(NEW), NOW()
    );
    RETURN NEW;
  ELSIF (TG_OP = 'UPDATE') THEN
    INSERT INTO audit_trail (
      organization_id, user_id, user_email, action, entity_type, entity_id, entity_code, old_values, new_values, timestamp
    ) VALUES (
      NEW.organization_id, auth.uid(), (SELECT email FROM auth.users WHERE id = auth.uid()),
      'update', 'risk', NEW.id, NEW.risk_code, to_jsonb(OLD), to_jsonb(NEW), NOW()
    );
    RETURN NEW;
  ELSIF (TG_OP = 'DELETE') THEN
    INSERT INTO audit_trail (
      organization_id, user_id, user_email, action, entity_type, entity_id, entity_code, old_values, timestamp
    ) VALUES (
      OLD.organization_id, auth.uid(), (SELECT email FROM auth.users WHERE id = auth.uid()),
      'delete', 'risk', OLD.id, OLD.risk_code, to_jsonb(OLD), NOW()
    );
    RETURN OLD;
  END IF;
END;
...
-- ============================================================================
-- PART 10: RLS POLICIES (CORRECTED)
-- ============================================================================
...
CREATE POLICY "Users can view their own profile" ON user_profiles FOR SELECT TO authenticated USING (id = auth.uid());
...
CREATE POLICY "Users can update their own profile" ON user_profiles FOR UPDATE TO authenticated USING (id = auth.uid());

-- Risks (USER + ADMIN policies)
DROP POLICY IF EXISTS "Users can view their own risks" ON risks;
CREATE POLICY "Users can view their own risks" ON risks FOR SELECT TO authenticated USING (user_id = auth.uid());
...
CREATE POLICY "Users can insert their own risks" ON risks FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid() AND organization_id = current_org_id());
...
CREATE POLICY "Users can update their own risks" ON risks FOR UPDATE TO authenticated USING (user_id = auth.uid());
...
CREATE POLICY "Users can delete their own risks" ON risks FOR DELETE TO authenticated USING (user_id = auth.uid());
...
-- Controls (USER + ADMIN policies)
DROP POLICY IF EXISTS "Users can view controls for their risks" ON controls;
CREATE POLICY "Users can view controls for their risks" ON controls FOR SELECT TO authenticated USING (risk_id IN (SELECT id FROM risks WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can insert controls for their risks" ON controls FOR INSERT TO authenticated WITH CHECK (risk_id IN (SELECT id FROM risks WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can update controls for their risks" ON controls FOR UPDATE TO authenticated USING (risk_id IN (SELECT id FROM risks WHERE user_id = auth.uid()));
...
CREATE POLICY "Users can delete controls for their risks" ON controls FOR DELETE TO authenticated USING (risk_id IN (SELECT id FROM risks WHERE user_id = auth.uid()));
...
-- Intelligence Alerts (Links to risks)
...
SELECT
  r.risk_code, r.risk_title, r.category, r.organization_id, COUNT(krl.kri_id) AS kri_count,
...
SELECT
  r.risk_code, r.risk_title, COUNT(ria.id) AS total_alerts,
...
BEGIN
  ...
  SELECT COALESCE(MAX(CAST(SUBSTRING(risk_code FROM LENGTH(v_prefix) + 2) AS INTEGER)), 0) INTO v_max_num
  FROM risks WHERE organization_id = p_organization_id AND risk_code LIKE v_prefix || '-%';
...
BEGIN
  UPDATE risk_intelligence_alerts SET status = 'expired' WHERE status = 'pending' AND created_at < NOW() - INTERVAL '30 days';
...
DO $$
BEGIN
  RAISE NOTICE '✅ MinRisk v4.0 Complete Schema Migration Successful (SAFE VERSION)!';
  RAISE NOTICE '📊 Created/Updated: 19 tables with proper RLS policies';
  RAISE NOTICE '🔒 CRITICAL FIX: Added admin policies for user-scoped tables';
