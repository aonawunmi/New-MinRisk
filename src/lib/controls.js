 ...
 */
async function generateControlCode(organizationId) {
    try {
 ...
 */
export async function getControlsForRisk(riskId) {
    try {
        const orgId = await getCurrentOrgId();
        if (!orgId) {
            return { data: null, error: new Error('No organization context') };
 ...
 */
export async function createControl(controlData) {
    try {
        const orgId = await getCurrentOrgId();
 ...
 */
export function calculateControlEffectiveness(design, implementation, monitoring, evaluation) {
    if (design === null ||
        implementation === null ||
        monitoring === null ||
        evaluation === null) {
 ...
 */
export async function calculateResidualRisk(riskId, inherentLikelihood, inherentImpact) {
    try {
 ...
 */
export async function getControlSummary(riskId, inherentLikelihood, inherentImpact) {
    try {
        // Get all controls for this risk
        const { data: controls, error: controlsError } = await getControlsForRisk(riskId);
        if (controlsError) {
            return { data: null, error: controlsError };
        }
        // Calculate residual risk
        const { data: residualRisk, error: residualError } = await calculateResidualRisk(riskId, inherentLikelihood, inherentImpact);
        if (residualError || !residualRisk) {
            return { data: null, error: residualError || new Error('No residual risk data') };
        }
        // Calculate summary statistics
        const likelihoodControls = (controls || []).filter((c) => c.target === 'Likelihood');
        const impactControls = (controls || []).filter((c) => c.target === 'Impact');
        // Calculate average effectiveness
        const effectivenessScores = (controls || [])
            .filter((c) => c.design_score !== null &&
            c.implementation_score !== null &&
            c.monitoring_score !== null &&
            c.evaluation_score !== null)
            .map((c) => calculateControlEffectiveness(c.design_score, c.implementation_score, c.monitoring_score, c.evaluation_score));
