...
export async function deleteCategory(id) {
    try {
        ...
        const { data: canDelete, error: checkError } = await supabase.rpc('can_delete_category', { category_id_param: id });
        if (checkError)
            throw checkError;
        if (!canDelete) {
            throw new Error('Cannot delete category: it has subcategories or is assigned to risks');
        }
 ...
 */
export async function updateSubcategory(updateData) {
    try {
        const { id, ...updates } = updateData;
        // Validate description length if provided
        if (updates.description && updates.description.length > 200) {
...
export async function deleteSubcategory(id) {
    try {
        ...
        const { data: canDelete, error: checkError } = await supabase.rpc('can_delete_subcategory', { subcategory_id_param: id });
        if (checkError)
            throw checkError;
        if (!canDelete) {
            throw new Error('Cannot delete subcategory: it is assigned to one or more risks');
        }
        const { error } = await supabase
            .from('risk_subcategories')
            .delete()
 ...
 */
export async function importTaxonomy(rows) {
    try {
        let imported = 0;
        let skipped = 0;
        const errors = [];
