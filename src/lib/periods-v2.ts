...
export interface ActivePeriod {
  ...
  current_period_quarter: number;
  previous_period_year: number | null;
  previous_period_quarter: number | null;
...
export interface RiskHistorySnapshot {
  ...
  change_type: 'PERIOD_COMMIT' | 'STATUS_CHANGE' | 'OWNER_CHANGE' | 'RATING_CHANGE';
...
export interface PeriodCommit {
  ...
  committed_by: string | null;
  risks_count: number;
  active_risks_count: number | null;
  closed_risks_count: number | null;
  controls_count: number | null;
  kris_count: number | null;
...
export interface PeriodComparison {
  ...
  risk_count_period1: number;
  risk_count_period2: number;
  new_risks: RiskHistorySnapshot[];
  closed_risks: RiskHistorySnapshot[];
  risk_changes: RiskChange[];
  score_changes: {
    avg_inherent_change: number;
    avg_residual_change: number;
    avg_inherent_period1: number;
    avg_inherent_period2: number;
    avg_residual_period1: number;
    avg_residual_period2: number;
  };
}
 ...
 */
export function parsePeriod(periodStr: string): Period | null {
 ...
 */
export async function getActivePeriod(orgId: string): Promise<{
...
export async function setActivePeriod(
  orgId: string,
  period: Period
): Promise<{ data: ActivePeriod | null; error: Error | null }> {
  try {
    const { data, error } = await supabase
      .from('active_period')
      .upsert({
...
export async function commitPeriod(
  ...
  userId: string,
  notes?: string
): Promise<{ data: PeriodCommit | null; error: Error | null }> {
  try {
    ...
    for (const risk of risks) {
      ...
      const { data: residualCalc } = await calculateResidualRisk(
        risk.id,                    // Correct: risk ID
    ...
    console.log(
      `✅ Period committed: ${formatPeriod(period)} → ${formatPeriod(nextPeriod)}`
    );
    console.log(
      `   ${risks.length} risks snapshotted (${activeCount} active, ${closedCount} closed)`
    );
 ...
 */
export async function getCommittedPeriods(orgId: string): Promise<{
  data: PeriodCommit[] | null;
  error: Error | null;
}> {
  try {
 ...
 */
export async function getRiskHistoryForPeriod(
  orgId: string,
  period: Period
): Promise<{ data: RiskHistorySnapshot[] | null; error: Error | null }> {
  try {
    const { data, error } = await supabase
      .from('risk_history')
      .select('*')
 ...
 */
export async function getRiskTimeline(riskId: string): Promise<{
  data: RiskHistorySnapshot[] | null;
  error: Error | null;
}> {
  try {
    const { data, error } = await supabase
 ...
 */
export async function compareSnapshotPeriods(
  orgId: string,
  period1: Period,
  period2: Period
  ...
  try {
    ...
    const { data: snapshot1, error: error1 } = await getRiskHistoryForPeriod(
      orgId,
      period1
    );
    const { data: snapshot2, error: error2 } = await getRiskHistoryForPeriod(
      orgId,
      period2
    );
    ...
    if (error1 || error2) {
      return {
        data: null,
        error: new Error(
          error1?.message || error2?.message || 'Failed to load snapshots'
        ),
      };
    }
    ...
    snapshot2.forEach((risk2) => {
      ...
      if (risk1) {
        const likelihoodChange =
          risk2.likelihood_inherent - risk1.likelihood_inherent;
    ...
    const avgInherent1 =
      snapshot1.reduce((sum, r) => sum + r.score_inherent, 0) /
      snapshot1.length;
    const avgInherent2 =
      snapshot2.reduce((sum, r) => sum + r.score_inherent, 0) /
      snapshot2.length;
    const avgResidual1 =
      snapshot1.reduce((sum, r) => sum + (r.score_residual || r.score_inherent), 0) /
      snapshot1.length;
    const avgResidual2 =
      snapshot2.reduce((sum, r) => sum + (r.score_residual || r.score_inherent), 0) /
      snapshot2.length;
 ...
 */
export function generatePeriodOptions(): Period[] {
...
export interface PeriodTrendData {
  ...
  // Individual level counts for charts
 ...
 */
export async function getPeriodTrends(orgId: string): Promise<{
  data: PeriodTrendData[] | null;
  error: Error | null;
}> {
  try {
...
export async function analyzeRiskMigrations(
  ...
  try {
    ...
    history2.forEach((risk2) => {
      ...
      if (!risk1) return; // Risk didn't exist in period 1
