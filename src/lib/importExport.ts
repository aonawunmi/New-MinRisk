 ...
 *
 * Handles CSV and Excel import/export for risks and other entities.
 * Uses Papa Parse for CSV, and would use XLSX.js for Excel (to be added).
 */
 ...
 */
export async function exportRisksToCSV(
...
function parseCSV(csvContent: string): {
  ...
  const rows = lines.slice(1).map((line, index) => {
    ...
    const row: Record<string, any> = { _rowNumber: index + 2 }; // +2 for header and 1-based
 ...
 */
function transformCSVRowToRisk(row: Record<string, any>): any {
 ...
 */
export async function importRisksFromCSV(
  csvContent: string
 ...
 */
export function generateRiskImportTemplate(): string {
  const headers = [
    'risk_code',
    'risk_title',
    'risk_description',
 ...
 */
export function downloadRiskImportTemplate(): void {
 ...
 */
export async function exportControlsToCSV(
  options?: ExportOptions
): Promise<{ data: string | null; error: Error | null }> {
  try {
...
function validateControlRow(
  ...
  if (row.control_type) {
    const validTypes: ControlType[] = ['preventive', 'detective', 'corrective'];
  ...
  // Validate target
  if (row.target) {
    const validTargets: ControlTarget[] = ['Likelihood', 'Impact'];
 ...
 */
async function transformCSVRowToControl(
  row: Record<string, any>,
  ...
  try {
    ...
    const parseScore = (value: any): DIMEScore | null => {
      if (value === null || value === undefined || value === '') return null;
      const score = parseInt(value);
      return isNaN(score) ? null : (score as DIMEScore);
    };
 ...
 */
export async function importControlsFromCSV(
  csvContent: string
): Promise<{ data: ImportResult | null; error: Error | null }> {
  try {
    // Get current user and org
    ...
    for (const row of rows) {
      try {
        const { data: controlData, error: transformError } =
          await transformCSVRowToControl(row, profile.organization_id);
 ...
 */
export function generateControlImportTemplate(): string {
  const headers = [
    'control_code',
    'risk_code',
    'name',
 ...
 */
export function downloadControlImportTemplate(): void {
