 ...
 */
async function generateKRICode(organizationId) {
    try {
...
export async function getKRIDefinitions() {
    try {
      ...
      `)
        ...
        const krisWithLinks = data?.map((kri) => ({
            ...kri,
            linked_risk_codes: kri.kri_risk_links?.map((link) => link.risks?.risk_code).filter(Boolean) || [],
 ...
 */
export async function getKRIDefinition(kriId) {
    try {
        const { data, error } = await supabase
            .from('kri_definitions')
            .select('*')
 ...
 */
export async function createKRI(kriData) {
    try {
        // Get current user
 ...
 */
export async function updateKRI(kriData) {
    try {
 ...
 */
export async function deleteKRI(kriId) {
    try {
 ...
 */
export async function getKRIDataEntries(kriId, limit) {
    try {
        let query = supabase
 ...
 */
function calculateAlertStatus(value, kri) {
    if (!kri.threshold_direction)
        return 'green';
 ...
 */
export async function createKRIDataEntry(entryData) {
    try {
        // Get current user
        const { data: { user }, error: userError, } = await supabase.auth.getUser();
        if (userError || !user) {
        ...
        const { data: kri, error: kriError } = await getKRIDefinition(entryData.kri_id);
        if (kriError || !kri) {
        ...
        if (alertStatus === 'yellow' || alertStatus === 'red') {
            await createKRIAlert({
                ...
                threshold_breached: alertStatus === 'red'
 ...
 */
export async function getKRIAlerts(status) {
    try {
        let query = supabase
            .from('kri_alerts')
            .select(`
      ...
      `)
        ...
        const flattenedData = data?.map((alert) => ({
            ...alert,
 ...
 */
export async function getAllOpenKRIAlerts() {
    try {
        const { data, error } = await supabase
            .from('kri_alerts')
            .select(`
 ...
 */
export async function acknowledgeKRIAlert(alertId, notes) {
    try {
        const { data: { user }, error: userError, } = await supabase.auth.getUser();
        if (userError || !user) {
 ...
 * Resolve a KRI alert
 */
export async function resolveKRIAlert(alertId, resolutionNotes) {
    try {
        const { data: { user }, error: userError, } = await supabase.auth.getUser();
        if (userError || !user) {
            return { error: new Error('User not authenticated') };
 ...
 */
export async function linkKRIToRisk(kriId, riskCodeOrId, // Can accept either risk_code or risk_id
aiConfidence) {
    try {
        const { data: { user }, error: userError, } = await supabase.auth.getUser();
        if (userError || !user) {
 ...
 */
export async function getKRIsForRisk(riskCodeOrId // Can accept either risk_code or risk_id
) {
    try {
        // Check if input is UUID (risk_id) or code (risk_code)
        let riskId = riskCodeOrId;
 ...
 */
export async function unlinkKRIFromRisk(linkId) {
    try {
        const { error } = await supabase
            .from('kri_risk_links')
            .delete()
 ...
 */
export async function getRisksWithoutKRICoverage() {
    try {
        // Get all risks
        ...
        const uncoveredRisks = risks?.filter((r) => !linkedRiskIds.has(r.id));
 ...
 */
export async function getKRICoverageStats() {
    try {
        // Get total risks
        ...
        const coveragePercentage = totalRisks && totalRisks > 0
 ...
 */
export async function generateAIKRISuggestions(riskCode) {
    try {
