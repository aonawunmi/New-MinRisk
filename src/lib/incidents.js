 ...
 */
export async function getUserIncidents(filters) {
    try {
        let query = supabase
            .from('incidents')
            .select('*')
 ...
 */
export async function updateIncident(incidentId, input) {
    try {
        const { data, error } = await supabase
            .from('incidents')
            .update({
 ...
 * Only for admins
 */
export async function updateIncidentDescription(incidentId, newDescription, reason) {
    try {
        // Get current user
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
 ...
 */
export async function linkIncidentToRisk(incidentId, riskId, mappingSource, reason, confidenceScore) {
    try {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
            throw new Error('Not authenticated');
 ...
 */
export async function unlinkIncidentFromRisk(incidentId, reason) {
    try {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
            throw new Error('Not authenticated');
 ...
 */
export async function analyzeIncidentForRiskMapping(incidentId) {
    try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session)
            throw new Error('Not authenticated');
        console.log(`🧠 Triggering AI analysis for incident: ${incidentId}`);
        const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/analyze-incident-for-risk-mapping`, {
            method: 'POST',
            headers: {
 ...
 */
export async function getAISuggestionsForIncident(incidentId, status) {
    try {
        let query = supabase
 ...
 */
export async function acceptAISuggestion(suggestionId, linkType, // NEW: Admin selects link type
adminNotes, classificationConfidence) {
    try {
 ...
 */
export async function rejectAISuggestion(suggestionId, adminNotes) {
    try {
 ...
 */
export async function createIncidentRiskLink(incidentId, riskId, linkType, adminNotes, classificationConfidence) {
    try {
 ...
 */
export async function deleteIncidentRiskLink(incidentId, riskId, adminNotes) {
    try {
 ...
 */
export async function getIncidentRiskLinks(incidentId) {
    try {
        const { data, error } = await supabase
            .from('incident_risk_links')
            .select(`
 ...
 */
export async function getIncidentMappingHistory(incidentId) {
    try {
        const { data, error } = await supabase
            .from('incident_risk_mapping_history')
            .select(`
 ...
 */
export async function voidIncident(incidentId, reason) {
    try {
 ...
 */
export async function getIncidentLifecycleHistory(incidentId) {
    try {
        const { data, error } = await supabase
 ...
 */
export async function getIncidentsForRisk(riskId) {
    try {
        const { data, error } = await supabase
            .from('incident_risk_links')
            .select(`
      ...
      `)
        ...
        const filteredData = data?.filter(link => link.incidents && link.incidents.incident_status === 'ACTIVE');
