 ...
 */
async function generateRiskCode(organizationId, division, category) {
    try {
...
export async function getRiskById(riskId) {
    try {
      ...
      `)
        ...
        const controls = controlLinks?.map((link) => link.controls).filter(Boolean) || [];
        return { data: { ...risk, controls }, error: null };
    }
    catch (err) {
...
export async function createRisk(riskData) {
    try {
        ...
        const riskCode = riskData.risk_code || await generateRiskCode(profile.organization_id, riskData.division, riskData.category);
        const { data, error } = await supabase
            .from('risks')
            .insert([
            {
 ...
 */
export async function updateRisk(riskData) {
    try {
 ...
 */
export async function addControl(riskId, controlData) {
    try {
        // Get current user
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        if (userError || !user) {
 ...
 */
export async function linkControlToRisk(controlId, riskId) {
    try {
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        if (userError || !user) {
 ...
 */
export async function unlinkControlFromRisk(controlId, riskId) {
    try {
        const { error } = await supabase
            .from('risk_control_links')
            .delete()
 ...
 */
export async function linkKRIToRisk(kriId, riskId, aiConfidence) {
    try {
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        if (userError || !user) {
            return { error: new Error('User not authenticated') };
 ...
 */
export async function unlinkKRIFromRisk(kriId, riskId) {
    try {
        const { error } = await supabase
            .from('kri_risk_links')
            .delete()
 ...
 */
export async function getRisksForControl(controlId) {
    try {
      ...
      `)
        ...
        const risks = riskLinks?.map((link) => link.risks).filter(Boolean) || [];
