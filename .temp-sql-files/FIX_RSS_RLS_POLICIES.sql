-- Fix RLS policies for rss_sources table
...
-- Root cause: user_id vs id column mismatch in policies
...
-- Create corrected policies with proper column references
...
CREATE POLICY "Users can view organization RSS sources"
  ...
  USING (
    organization_id IN (
      ...
      WHERE user_id = auth.uid()  -- Changed from 'id' to 'user_id'
    )
  );
