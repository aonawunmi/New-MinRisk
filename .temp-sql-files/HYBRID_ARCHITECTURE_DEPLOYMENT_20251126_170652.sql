...
-- ============================================================================
--
-- Generated: Wed Nov 26 17:06:52 WAT 2025
--
...
CREATE INDEX IF NOT EXISTS idx_global_control_dime_avg ON global_control_library((design_score + implementation_score + monitoring_score + evaluation_score) / 4);

-- ============================================================================
-- PART 2: MIGRATE EXISTING CONTROL LIBRARY DATA TO GLOBAL TABLE
-- ============================================================================
...
SELECT DISTINCT ON (control_code)
  ...
  industry_standards,
  automation_level,
  'Intermediate' as complexity_level -- Default, can be updated
...
-- ============================================================================
-- PART 2: MIGRATE EXISTING KRI/KCI DATA TO GLOBAL TABLE
-- ============================================================================
...
-- ============================================================================
-- PART 6: MIGRATE EXISTING MAPPINGS TO GLOBAL TABLES
-- ============================================================================

-- Migrate Root Cause → KRI mappings (if they exist in old org-specific tables)
...
WHERE EXISTS (SELECT 1 FROM root_cause_kri_mapping) -- Only if table exists
ON CONFLICT (global_root_cause_id, global_kri_id) DO NOTHING;

-- Migrate Impact → KCI mappings
...
WHERE EXISTS (SELECT 1 FROM impact_kci_mapping) -- Only if table exists
...
CREATE TABLE IF NOT EXISTS control_dependencies (
  ...
  rationale TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(COALESCE(organization_id, '00000000-0000-0000-0000-000000000000'::UUID), control_id, depends_on_control_id)
);
